...
export class BookmarkView {
  ...
  public async currentItems() {
    if (this._currentItems && this._currentItems.length > 0) {
      return Object.assign([], this._currentItems);
    }
    ...
    const projectBookmarks =
      ext.getSetting<Bookmark[]>(SETTING.bookmarks, "project") || [];
    const globalBookmarks =
      ext.getSetting<Bookmark[]>(SETTING.bookmarks, "global") || [];
    this._currentItems = [
      ...this.validateBookmarks(projectBookmarks, false),
      ...this.validateBookmarks(globalBookmarks, true),
    ];
    return Object.assign([], this._currentItems);
  }
  ...
  public async bindbookmarks() {
    ...
    this.tree.onDidExpandElement((e) => {
      this.provider?.updateCollapsibleState(
        e.element,
  ...
  public async getBookmarks() {
    ...
    if (this.viewType === BookmarkViewType.team) {
      ...
      const projectGroups =
        ext.getSetting<Group[]>(SETTING.groups, "project") || [];
      const projectBookmarks =
        ext.getSetting<Bookmark[]>(SETTING.bookmarks, "project") || [];
      ...
      bookmarks = [
        ...projectBookmarks.map((b) => ({ ...b, isGlobal: false })),
    ...
    if (this.viewType === BookmarkViewType.project) {
      ...
      await commands.executeCommand(
        "setContext",
        CONTEXT_KEY.hasGlobalBookmarks,
        bookmarks.filter((b) => b.isGlobal).length > 0
      );
    }

    if (bookmarks.filter((b) => b.isGlobal).length > 0) {
      const globalGroups =
        ext.getSetting<Group[]>(SETTING.groups, "global") || [];

      const globalItems = await this.processBookmarks(
        bookmarks.filter((b) => b.isGlobal),
        globalGroups,
        true
      );
      const projectItems = await this.processBookmarks(
        bookmarks.filter((b) => !b.isGlobal),
        groups,
        false
      );
      ...
      const globalGroup = new BookmarkTreeItem(
        ...
        "Global bookmarks",
        undefined,
        await BookmarkProvider.getCollapsibleState(
          "global",
          BookmarkViewType.project
        ),
        true,
        new ThemeIcon("globe"),
        undefined,
      ...
      this._currentItems = [
        ...projectItems.bookmarks,
        ...globalItems.bookmarks,
      ];
  ...
  private async processBookmarks(
    ...
    let crntTreeItems = [
      ...crntBookmarks
        .filter((b) => !b.groupId && !b.isDeleted)
        .map((b) => createBookmark(b)),
    ];

    // Add all items assigned to a group
    for (const group of groups) {
      const groupItems = crntBookmarks.filter(
        (b) => b.groupId === group.id && !b.isDeleted
      );

      if (groupItems.length > 0) {
        const groupItem = await this.groupBookmarks(
          group,
          groupItems,
          this.viewType || BookmarkViewType.project,
          isGlobal
        );
        if (groupItem) {
  ...
  private validateBookmarks(bookmarks: Bookmark[], isGlobal: boolean = false) {
    return bookmarks.map((b) => {
      if (
        b.type === BookmarkType.File &&
        !existsSync(isGlobal ? b.path : toAbsPath(b.path).fsPath)
      ) {
        b.isDeleted = true;
      } else {
        delete b.isDeleted;
